...
class CalculateTaxView(APIView):
    def post(self, request):
        ...
        if request.data['whichSubmit'] == "submit2":
            ...
            timezone_offset_2 = timedelta(hours=5, minutes=30)  # +05:30 timezone offset
        ...
        headers = {
            "Content-Type": "application/json"  # Set the content type based on API requirements
        }
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    if isinstance(value, dict) or isinstance(value, list):
                        remove_quotes(value)  # Recursive call for nested dictionaries or lists
                ...
                for i in range(len(data)):
                    if isinstance(data[i], dict) or isinstance(data[i], list):
                        remove_quotes(data[i])  # Recursive call for nested dictionaries or lists
